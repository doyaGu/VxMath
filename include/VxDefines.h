...
public:
    explicit VxUV(float _u = 0, float _v = 0) : u(_u), v(_v) {}
...
struct VxDrawPrimitiveDataSimple
{
    ...
    unsigned int NormalStride;        // Amount in bytes between two positions in the NormalPtr buffer
    void *ColorPtr;                   // A pointer to colors data
    unsigned int ColorStride;         // Amount in bytes between two positions in the ColorPtr buffer
    void *SpecularColorPtr;           // A pointer to specular colors data
    unsigned int SpecularColorStride; // Amount in bytes between two positions in the SpecularColorPtr buffer
    void *TexCoordPtr;                // A pointer to texture coordinates data
    unsigned int TexCoordStride;      // Amount in bytes between two positions in the TexCoordPtr buffer
};
...
to specify vertices to transform and 2 arrays that will received the transformed homogenous
vertices and the screen coordinates of these vertices.

o All members except InVertices are optional.
...
typedef struct VxDirectXData
{
    ...
    XULONG DxVersion;      // DirectX Version (0x0500 for DirectX 5.0 , 0x0700 for DirectX 7, etc..)
} VxDirectXData;
...
Remarks:
+ The VXTEXTURE_ADDRESSMODE is used by CKMaterial::SetTextureAddressMode to specify texture coordinate are
...
typedef enum VXCMPFUNC
{
    ...
    VXCMP_LESS         = 2UL,   // Accept if value is less than current value.
    VXCMP_EQUAL        = 3UL,   // Accept if value is equal than current value.
    VXCMP_LESSEQUAL    = 4UL,   // Accept if value is less or equal than current value.
    VXCMP_GREATER      = 5UL,   // Accept if value is greater than current value.
    VXCMP_NOTEQUAL     = 6UL,   // Accept if value is different from current value.
    VXCMP_GREATEREQUAL = 7UL,   // Accept if value is greater or equal current value.
...
{
    XULONG ModulateColor;          // A DWORD ARGB Color to use if VXSPRITE_MODULATE is enabled
                                   // to multiply the sprite pixel
    XULONG Options : 4;            // A combinaison of VXSPRITE_RENDEROPTIONS used to render the sprite
    ...
    XULONG Options2 : 4;           // A combinaison of VXSPRITE_RENDEROPTIONS2 used to render the sprite
    VXBLEND_MODE DstBlendMode : 8; // If blending is enabled (VXSPRITE_BLEND), destination blend mode
    XBYTE AlphaRefValue : 8;       // If alpha test is enabled (VXSPRITE_ALPHATEST), reference value
};
...
typedef struct Vx2DCapsDesc
{
    ...
    XULONG MaxVideoMemory;       // Maximum video memory (Minus size already allocated for current display)
    XULONG AvailableVideoMemory; // Available video memory
    XULONG Caps;                 // General Caps
} Vx2DCapsDesc;
...
typedef enum CKRST_MISCCAPS
{
    ...
    CKRST_MISCCAPS_CULLCCW    = 0x00000040UL    // The driver supports counterclockwise triangle culling.
} CKRST_MISCCAPS;
...
typedef enum CKRST_RASTERCAPS
{
    ...
    CKRST_RASTERCAPS_WBUFFER        = 0x00040000UL, // Depth buffering can be performed using w values (more precise).
