...
TEST_F(AlphaBlitTest, CopyAlphaArray_PreservesColor) {
    ...
    const XDWORD* original = reinterpret_cast<const XDWORD*>(colorCopy.Data());
    const XDWORD* result = reinterpret_cast<const XDWORD*>(buffer.Data());
...
TEST_F(AlphaBlitTest, AlphaOnSinglePixel) {
    ...
    XDWORD* pixel = reinterpret_cast<XDWORD*>(buffer.Data());
    *pixel = 0x00FFAA55;
...
TEST_F(AlphaBlitTest, AlphaBlit_AllWidths_UniformAlpha) {
    ...
    for (int width : widths) {
        ...
        XDWORD* pixels = reinterpret_cast<XDWORD*>(buffer.Data());
...
TEST_F(AlphaBlitTest, PremultiplyUnpremultiply_Roundtrip) {
    ...
    const XDWORD *origPixels = reinterpret_cast<const XDWORD *>(original.Data());
    for (int i = 0; i < width * height; ++i) {
...
TEST_F(AlphaBlitTest, SwapRedBlue_24BitRGB) {
    const int width = 1;
    const int height = 1;
    ImageBuffer buffer(width * height * 3);
    auto desc = ImageDescFactory::Create24BitRGB(width, height, buffer.Data());

    PatternGenerator::FillSolid24(buffer.Data(), width, height, 0x11, 0x22, 0x33);

    blitter.SwapRedBlue(desc);

    EXPECT_EQ(buffer[0], 0x11u);
    EXPECT_EQ(buffer[1], 0x22u);
    EXPECT_EQ(buffer[2], 0x33u);
}

TEST_F(AlphaBlitTest, SwapRedBlue_16BitFormats) {
    const int width = 3;
    const int height = 1;
    ...
    blitter.SwapRedBlue(rgb565);

    EXPECT_EQ(pixels565[0], 0x001Fu);
    EXPECT_EQ(pixels565[1], 0x07E0u);
    EXPECT_EQ(pixels565[2], 0xF800u);
    ...
    blitter.SwapRedBlue(argb1555);

    EXPECT_EQ(pixels1555[0], 0x801Fu)
        << "ARGB1555 alpha bit should be preserved while red and blue swap";
    EXPECT_EQ(pixels1555[1], 0x7C00u)
        << "ARGB1555 transparent alpha bit should be preserved while red and blue swap";
    ...
    blitter.SwapRedBlue(argb4444);

    EXPECT_EQ(pixels4444[0], 0xA00Fu)
        << "ARGB4444 alpha nibble should be preserved while red and blue swap";
    EXPECT_EQ(pixels4444[1], 0x0F00u)
        << "ARGB4444 zero alpha nibble should be preserved while red and blue swap";
}
...
TEST_F(AlphaBlitTest, MultiplyBlend_White) {
    ...
    XDWORD *dstPixels = reinterpret_cast<XDWORD *>(dstBuffer.Data());
    for (int i = 0; i < width * height; ++i) {
